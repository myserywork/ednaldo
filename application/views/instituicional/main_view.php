<!DOCTYPE html>
<html lang="pt-br" dir="ltr">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">

    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css" integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T" crossorigin="anonymous">
    <link rel="stylesheet" href="/resources/css/master.css?rand=<?= rand(0, 1000000); ?>">

    <title>Clínica Queiroga</title>

    <script src="https://kit.fontawesome.com/9a3cbf7949.js" crossorigin="anonymous"></script>

    <script type="text/javascript">
      if (location.protocol != 'https:')
      {
        location.href = 'https:' + window.location.href.substring(window.location.protocol.length);
      }
    </script>
  </head>
  <body>
    <header class="section-header">
      <nav class="section-navbar">
        <div class="navbar-inner container">
          <a href="#"><img src="/resources/img/logomarca.png"></a>
          <div class="nav-links">
            <a href="#sobre">Dr. Ednaldo Queiroga</a>
            <a href="#clinica">Clínica</a>
            <a href="#servicos">Serviços</a>
            <a href="#cursos">Cursos</a>
            <a href="/blog">Blog</a>
            <a href="#contato">Contato</a>
            <a href="#contato" class="destaque">Agende sua consulta</a>
          </div>
        </div>
      </nav>

      <div class="container noselect" style="padding-bottom: 100px">
        <h2>Nutrologia Esportiva e</h2>
        <h1>LONGEVIDADE</h1>
        <p>Nossa missão é mudar vidas através de um estilo saudável, focado na mudança de hábitos alimentares, emagrecimento, hipertrofia muscular, terapia hormonal e outros benefícios que melhoram a sua qualidade de vida de maneira individualizada. </p>
        <p>
          <a class="btn btn-social" href="https://www.facebook.com/drednaldoqueiroga" target="_blank"><i class="fab fa-facebook-square"></i></a>
          <a class="btn btn-social" href="https://www.instagram.com/drednaldoqueiroga/" target="_blank"><i class="fab fa-instagram"></i></a>
          <a class="btn btn-social" href="https://www.youtube.com/c/DrEdnaldoQueiroga" target="_blank"><i class="fab fa-youtube"></i></a>
        </p>
      </div>

      <div class="section-hero d-none d-lg-block noselect"><img src="/resources/img/hero.png"/></div>
    </header>

    <section class="section-sobre" id="sobre">
      <div class="section-background" style="background-image: url('/resources/img/vector-bg.png')"></div>

      <div class="img-mobile-container d-lg-none">
        <div id="carousel2a" class="carousel slide" data-ride="carousel">
          <ol class="carousel-indicators">
            <li data-target="#carousel2a" data-slide-to="0" class="active"></li>
            <li data-target="#carousel2a" data-slide-to="1"></li>
          </ol>
          <div class="carousel-inner">
            <div class="carousel-item active">
              <div class="image" style="background-image: url('/resources/img/dr-queiroga.jpg')"></div>
            </div>
            <div class="carousel-item">
              <div class="image" style="background-image: url('/resources/img/dr-queiroga-2.jpg')"></div>
            </div>
          </div>
          <a class="carousel-control-prev" href="#carousel2a" role="button" data-slide="prev">
            <span class="carousel-control-prev-icon" aria-hidden="true"></span>
            <span class="sr-only">Anterior</span>
          </a>
          <a class="carousel-control-next" href="#carousel2a" role="button" data-slide="next">
            <span class="carousel-control-next-icon" aria-hidden="true"></span>
            <span class="sr-only">Próximo</span>
          </a>
        </div>
      </div>

      <div class="container">
        <div class="row">

          <div class="col-lg-6" style="padding: 54px 35px">
            <h1>
              Sobre o<br>
              <span style="font-weight: bold;">Dr. Ednaldo Queiroga</span>
            </h1>

            <div class="tabs" id="tabs2">
              <div>
                <p class="lead text-left font-weight-bold">
                  Nutrologia Esportiva e Longevidade<br>
                  <span style="font-size: 70%">CRM-PB 9167 | CRM-PE 22453 | CRM-RN 9422</span>
                </p>

                <p>
                  O Médico Dr. Ednaldo Queiroga possui em seu sangue a verdadeira paixão pelo desempenho da atividade médica.
                </p>

                <p>
                  Escritor de diversos Capítulos de Livros sobre Nutrologia e Clínica Médica, atualmente é Diretor Clínico da Clínica Queiroga, onde acompanha atletas profissionais, praticantes de atividade física em geral, portadores de desordens crônicas e pessoas em busca de qualidade de vida.
                </p>
                <p>
                  Atua como 1º Tenente do Exército no Hospital de Guarnição de Natal - HGuN na condição de Chefe da UTI do Ministério da Defesa, Médico Perito da Organização Militar (MPOM) e de Guarnição (MPGu), foi designado como Coordenador da Equipe Multiprofissional de Terapia Nutricional (EMTN).
                </p>
              </div>
              <div style="display: none">
                <ul>
                  <li>
                    Graduado em Medicina pela Faculdade de Medicina Nova Esperança - PB (FAMENE).
                  </li>
                  <li>
                    Pós-Graduado em Medicina Intensiva pela Faculdade Redentor - FACREDENTOR, vinculada à Associação de Medicina Intensiva Brasileira - AMIB.
                  </li>
                  <li>
                    Pós-Graduado em Aplicações Complementares às Ciências Militares com ênfase em Gerenciamento de Catástrofes em Situações de Uso de Armas Químicas, Biológicas, Radiológicas e Nucleares; Aprovado em Curso Internacional de referência na área militar mencionada, FDM - Fundamental Disaster Management, ministrado pela AMIB em associação à Society of Critical Care Medicine - SCCM.
                  </li>
                  <li>
                    Pós-Graduado em Nutrologia pela Associação Brasileira de Nutrologia - ABRAN.
                  </li>
                  <li>
                    Pós-Graduado em Nutrigenômica e Nutrigenética na Prática Clínica pela Faculdade Unyleya - UNYLEYA.
                  </li>
                  <li>
                    Pós-Graduando em Endocrinologia pelo Centro Brasileiro de Pós-Graduações - CENBRAP.
                  </li>
                  <li>
                    Pós-Graduando em Medicina do Exercício e do Esporte pelo HZM - Instituto de Ensino e Pesquisa.
                  </li>
                </ul>
              </div>
              <div style="display: none">
                <p>
                  Aprovado e Classificado em Primeiro Lugar na Seleção Especial de Médicos da 7ª Região Militar do Exército Brasileiro, concluiu com aproveitamento o Estágio de Adaptação e Serviço no 15º Batalhão de Infantaria Motorizado - 15º BIMtz, sendo diplomado como Melhor Combatente e Membro Honorário do Regimento Vidal de Negreiros.
                </p>
                <p>
                  Aprovado e Classificado entre os três Intensivistas do país no Concurso de Admissão 2016 ao Curso de Formação de Oficiais 2017 da Escola de Saúde do Exército - EsSEx.
                </p>
                <p class="lead text-left font-weight-bold">
                  Membro da:
                </p>
                <ul>
                  <li>
                    Associação de Medicina Intensiva Brasileira - AMIB.
                  </li>
                  <li>
                    Associação Brasileira de Nutrologia - ABRAN.
                  </li>
                  <li>
                    Sociedade Brasileira de Estudos da Fisiologia - SOBRAF.
                  </li>
                  <li>
                    Associação Brasileira para o Estudo da Obesidade e da Síndrome Metabólica - ABESO.
                  </li>
                  <li>
                    Sociedade Brasileira de Diabetes - SBD.
                  </li>
                  <li>
                    Academia Brasileira de Medicina Antienvelhecimento - ABMAE.
                  </li>
                  <li>
                    The American Association of Clinical Endocrinologists - AACE.
                  </li>
                  <li>
                    American College of Sports Medicine - ACSM.
                  </li>
                </ul>
              </div>
              <div style="display: none">
                <p class="lead text-left font-weight-bold">
                  Palestrante Internacional
                </p>
                <p>
                  Palestrante Internacional, tendo Ministrado o Curso Modulação Hormonal através da SPMECC - Sociedade Portuguesa de Medicina Estética e Cirurgia Cosmética na cidade do Porto - Portugal e Palestrado em grandes eventos como o 1º Congresso Nacional de Estética, Beleza e Saúde que teve lugar na Expocosmética e Health For Beauty na Exponor (maior evento de beleza da Península Ibérica) na cidade do Porto - Portugal e no 3º Congresso Nacional de Medicina Estética através da SPMECC na cidade do Porto - Portugal.
                </p>
                <p class="lead text-left font-weight-bold">
                  Colunista/Speaker
                </p>
                <p>
                  Em 2018 o Dr. Ednaldo Queiroga se tornou Colunista e Speaker da Vitafor Nutrientes (A Marca de Suplementos Alimentares mais Premiada pelos Profissionais da Saúde no Brasil), onde produz conhecimento no âmbito da saúde com a finalidade de gerar maior bem-estar para seus pacientes e clientes da empresa.
                </p>
              </div>
            </div>

            <script>
              function alterarTexto2(i) {
                $("#tabs2 > div").hide();
                $("#tabs2 > div").eq(i).show();
                $("#btns2 > button").removeClass('active');
                $("#btns2 > button").eq(i).addClass('active');
              }
            </script>

            <div class="d-flex flex-wrap justify-content-center justify-content-lg-start" id="btns2">
              <button type="button" class="btn-cicle active" style="margin-bottom: 38px; margin-right: 38px" onclick="alterarTexto2(0)"></button>
              <button type="button" class="btn-cicle" style="margin-bottom: 38px; margin-right: 38px" onclick="alterarTexto2(1)"></button>
              <button type="button" class="btn-cicle" style="margin-bottom: 38px; margin-right: 38px" onclick="alterarTexto2(2)"></button>
              <button type="button" class="btn-cicle" style="margin-bottom: 38px; " onclick="alterarTexto2(3)"></button>
            </div>
          </div>
          <div class="col-lg-6 d-none d-lg-block img-container">
            <div id="carousel2b" class="carousel slide" data-ride="carousel">
              <ol class="carousel-indicators">
                <li data-target="#carousel2b" data-slide-to="0" class="active"></li>
                <li data-target="#carousel2b" data-slide-to="1"></li>
              </ol>
              <div class="carousel-inner">
                <div class="carousel-item active">
                <div class="image" style="background-image: url('/resources/img/dr-queiroga.jpg')"></div>

                </div>
                <div class="carousel-item">
                  <div class="image" style="background-image: url('/resources/img/dr-queiroga-2.jpg'); background-position: top"></div>

                </div>
              </div>
              <a class="carousel-control-prev" href="#carousel2b" role="button" data-slide="prev">
                <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                <span class="sr-only">Anterior</span>
              </a>
              <a class="carousel-control-next" href="#carousel2b" role="button" data-slide="next">
                <span class="carousel-control-next-icon" aria-hidden="true"></span>
                <span class="sr-only">Próximo</span>
              </a>
            </div>
          </div>
        </div>
      </div>

    </section>

    <section class="section-sobre clinica" id="clinica">
      <div class="section-background" style="background-image: url('/resources/img/vector-bg.png')"></div>

      <div class="img-mobile-container d-lg-none">
        <div id="carousel1a" class="carousel slide" data-ride="carousel">
          <ol class="carousel-indicators">
            <li data-target="#carousel1a" data-slide-to="0" class="active"></li>
            <li data-target="#carousel1a" data-slide-to="1"></li>
          </ol>
          <div class="carousel-inner">
            <div class="carousel-item active">
              <div class="image" style="background-image: url('/resources/img/clinica-1.jpg')"></div>

            </div>
            <div class="carousel-item">
              <div class="image" style="background-image: url('/resources/img/clinica-2.jpg')"></div>

            </div>
          </div>
          <a class="carousel-control-prev" href="#carousel1a" role="button" data-slide="prev">
            <span class="carousel-control-prev-icon" aria-hidden="true"></span>
            <span class="sr-only">Anterior</span>
          </a>
          <a class="carousel-control-next" href="#carousel1a" role="button" data-slide="next">
            <span class="carousel-control-next-icon" aria-hidden="true"></span>
            <span class="sr-only">Próximo</span>
          </a>
        </div>
      </div>

      <div class="container">
        <div class="row">
          <div class="col-lg-6 d-none d-lg-block img-container">
            <div id="carousel1b" class="carousel slide" data-ride="carousel">
              <ol class="carousel-indicators">
                <li data-target="#carousel1b" data-slide-to="0" class="active"></li>
                <li data-target="#carousel1b" data-slide-to="1"></li>
              </ol>
              <div class="carousel-inner">
                <div class="carousel-item active">
                  <div class="image" style="background-image: url('/resources/img/clinica-1.jpg')"></div>
                </div>
                <div class="carousel-item">
                  <div class="image" style="background-image: url('/resources/img/clinica-2.jpg')"></div>
                </div>
              </div>
              <a class="carousel-control-prev" href="#carousel1b" role="button" data-slide="prev">
                <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                <span class="sr-only">Anterior</span>
              </a>
              <a class="carousel-control-next" href="#carousel1b" role="button" data-slide="next">
                <span class="carousel-control-next-icon" aria-hidden="true"></span>
                <span class="sr-only">Próximo</span>
              </a>
            </div>
          </div>

          <div class="col-lg-6" style="padding: 54px 35px">
            <h1>
              Sobre a<br>
              <span style="font-weight: bold;">Clínica Queiroga</span>
            </h1>

            <div class="tabs" id="tabs1">
              <div>
                <p>
                  Situada na cidade de João Pessoa - PB, próxima à Praia de Manaíra, a Clínica Queiroga traz um novo conceito em atendimento, visando a saúde e a qualidade de vida.
                </p>

                <p>
                  Buscamos arduamente a excelência profissional, aplicando conceitos atualizados da ciência, criando dentro de um único espaço uma composição harmoniosa entre as especialidades.
                </p>

                <p>
                  Nossa missão é mudar vidas através de um estilo saudável, focado na mudança de hábitos alimentares, emagrecimento, hipertrofia muscular, terapia hormonal e outros benefícios que melhoram a sua qualidade de vida de maneira individualizada.
                </p>
                <p>
                  A Clínica Queiroga hoje conta com uma equipe Multiprofissional focada na saúde, bem-estar e satisfação do paciente, visando sempre o equilíbrio físico e emocional, bem como a segurança do mesmo. Especializada em Nutrologia Esportiva e Longevidade, a Clínica conta com diferentes profissionais na área de saúde, como Médico, Nutricionista e Psicóloga.
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>

    </section>

    <section class="section-servicos" id="servicos">
      <div class="section-background" style="background-image: url('/resources/img/vector-logo.png')"></div>

      <div class="container">
        <h1 class="text-center">
          Confira todos os<br>
          <span style="font-weight: bold;">Nossos Serviços</span>
        </h1>

        <div class="row">
          <div class="col-lg-8">
            <div class="servico-box card">
              <div class="row" style="padding: 46px 0px">
                <div class="col-12 col-sm-auto">
                  <div class="servico-picture" style="background-image: url('/resources/img/servicos/1.jpg')"></div>
                </div>
                <div class="col-12 col-sm">
                  <p class="servico-description">
                    <span class="servico-title">Bioimpedância</span>
                    A Bioimpedância é um exame que analisa a composição corporal. Por meio deste exame medimos a quantidade de gordura, músculo, osso, água corporal, idade metabólica, entre outros dados que facilitarão a escolha do tipo de dieta e individualização das demais possibilidades terapêuticas.
                  </p>
                </div>
              </div>
            </div>
          </div>

          <div class="col-12"></div>

          <div class="col-lg-8  ml-auto">
            <div class="servico-box">
              <div class="row" style="padding: 46px 0px">
                <div class="col-12 col-sm-auto">
                  <div class="servico-picture" style="background-image: url('/resources/img/servicos/2.jpg')"></div>
                </div>
                <div class="col-12 col-sm">
                  <p class="servico-description">
                    <span class="servico-title">Exames Genéticos</span>
                    A Clínica Queiroga é Pioneira em realização e análise de Exames Genéticos na Paraíba, possibilitando a identificação de mais de 100 polimorfismos. O Dr. Ednaldo Queiroga é Credenciado e Habilitado a realizar Exames Genéticos pela DF Medica, a maior empresa Europeia de Biotecnologia. Os exames são baseados em Mapeamento Genético, atuando de maneira personalizada na criação do plano dietético, prescrição nutracêutica, recomendação do tipo mais adequado de atividade física e identificação de predisposição a determinadas doenças.
                  </p>
                </div>
              </div>
            </div>
          </div>

          <div class="col-12"></div>

          <div class="col-lg-8 ">
            <div class="servico-box card">
              <div class="row" style="padding: 46px 0px">
                <div class="col-12 col-sm-auto">
                  <div class="servico-picture" style="background-image: url('/resources/img/servicos/3.jpg')"></div>
                </div>
                <div class="col-12 col-sm">
                  <p class="servico-description">
                    <span class="servico-title">Equilíbrio Metabólico e Hormonal</span>
                    Tratamento da Obesidade, Diabetes, das doenças relacionadas à glândula Tireoide, Menopausa, Hipogonadismo e outras doenças relacionadas à deficiência ou excesso de hormônios.
                  </p>
                </div>
              </div>
            </div>
          </div>

          <div class="col-12"></div>

          <div class="col-lg-8  ml-auto">
            <div class="servico-box">
              <div class="row" style="padding: 46px 0px">
                <div class="col-12 col-sm-auto">
                  <div class="servico-picture" style="background-image: url('/resources/img/servicos/4.jpg')"></div>
                </div>
                <div class="col-12 col-sm">
                  <p class="servico-description">
                    <span class="servico-title">Protocolos Injetáveis</span>
                    Os Protocolos Injetáveis (intramusculares ou subcutâneos) são utilizados com diversas finalidades, como otimização do metabolismo em processos de emagrecimento, hipertrofia muscular, melhora da performance esportiva, reposição de vitaminas, combate à sarcopenia (perda muscular do idoso), melhora da cognição, entre outras indicações.
                  </p>
                </div>
              </div>
            </div>
          </div>

          <div class="col-12"></div>

          <div class="col-lg-8 ">
            <div class="servico-box card">
              <div class="row" style="padding: 46px 0px">
                <div class="col-12 col-sm-auto">
                  <div class="servico-picture" style="background-image: url('/resources/img/servicos/5.jpg')"></div>
                </div>
                <div class="col-12 col-sm">
                  <p class="servico-description">
                    <span class="servico-title">Nutrição</span>
                    Nosso Nutricionista trabalha através da prescrição de cardápio personalizado de acordo com os objetivos e necessidades de cada paciente, respeitando a individualidade biológica. Dispomos de Testes para identificação de Intolerâncias Alimentares que podem ser realizados em laboratório conveniado, ajudando e direcionando a prescrição do cardápio.
                  </p>
                </div>
              </div>
            </div>
          </div>

          <div class="col-12"></div>

          <div class="col-lg-8  ml-auto">
            <div class="servico-box">
              <div class="row" style="padding: 46px 0px">
                <div class="col-12 col-sm-auto">
                  <div class="servico-picture" style="background-image: url('/resources/img/servicos/6.jpg')"></div>
                </div>
                <div class="col-12 col-sm">
                  <p class="servico-description">
                    <span class="servico-title">Emagrecimento</span>
                    Através de criteriosa avaliação clínica e laboratorial atuamos nas diversas causas do Sobrepeso e Obesidade, com um tratamento individualizado focado na saúde e nos resultados. Utilizamos apenas tratamentos com respaldo científico. Metas a curto e longo prazo serão estabelecidas. O tratamento terá como base as mudanças de hábitos que cursam com ganho de peso, mudanças de comportamento e atividade física. Medicamentos podem e devem ser utilizados quando bem indicados.
                  </p>
                </div>
              </div>
            </div>
          </div>

          <div class="col-12"></div>

          <div class="col-lg-8 ">
            <div class="servico-box card">
              <div class="row" style="padding: 46px 0px">
                <div class="col-12 col-sm-auto">
                  <div class="servico-picture" style="background-image: url('/resources/img/servicos/7.jpg')"></div>
                </div>
                <div class="col-12 col-sm">
                  <p class="servico-description">
                    <span class="servico-title">Hipertrofia Muscular</span>
                    A Hipertrofia Muscular requer disciplina e dedicação. A base para o êxito consiste em treinos intensos e eficazes, dieta bem calculada e descanso adequados. Níveis hormonais otimizados são muito importantes. Contamos com diversos protocolos com medicações manipuladas via oral e injetáveis que auxiliarão no aumento da taxa de síntese proteica.
                  </p>
                </div>
              </div>
            </div>
          </div>

          <div class="col-12"></div>

          <div class="col-lg-8  ml-auto">
            <div class="servico-box">
              <div class="row" style="padding: 46px 0px">
                <div class="col-12 col-sm-auto">
                  <div class="servico-picture" style="background-image: url('/resources/img/servicos/8.jpg')"></div>
                </div>
                <div class="col-12 col-sm">
                  <p class="servico-description">
                    <span class="servico-title">Performance Esportiva</span>
                    O melhor desempenho para o seu corpo. Um planejamento dietético que se adeque às suas necessidades e rotina de treinos será elaborado. Deficiências vitamínicas e déficits hormonais serão corrigidos. As melhores estratégias de suplementação serão prescritas, baseando-se no que os artigos científicos trazem de mais moderno. O seu desempenho será otimizado ao máximo.
                  </p>
                </div>
              </div>
            </div>
          </div>

          <div class="col-12"></div>

          <div class="col-lg-8 ">
            <div class="servico-box card">
              <div class="row" style="padding: 46px 0px">
                <div class="col-12 col-sm-auto">
                  <div class="servico-picture" style="background-image: url('/resources/img/servicos/9.jpg')"></div>
                </div>
                <div class="col-12 col-sm">
                  <p class="servico-description">
                    <span class="servico-title">Longevidade</span>
                    Envelhecer é inevitável, mas podemos optar entre o envelhecimento ativo e saudável ou o envelhecimento limitado e dependente. Esse futuro depende das nossas escolhas hoje. A tão sonhada Longevidade Saudável está em nossas mãos e cabe a nós decidir qual caminho queremos percorrer.
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>

    <section class="section-cursos container" id="cursos">
      <h1 class="mt-5 text-center">
        Conheça todos os<br>
        <span style="font-weight: bold">Nossos Cursos</span>
      </h1>
      <div class="row">
        <div class="col-lg-6" style="padding-top: 130px; padding-bottom: 63px">
          <h5>Hormônios - Do Empirismo à Terapia Individualizada</h5>

          <p>
            <br>
            “Jamais considere seus estudos como uma obrigação, mas como uma oportunidade invejável para aprender a conhecer a influência libertadora da beleza do reino do espírito, para seu próprio prazer pessoal e para proveito da comunidade à qual seu futuro trabalho pertencer.”
            <br><br>
            (Albert Einstein)
          </p>

          <a href="/site#contato" class="btn btn-primary">Confira!</a>
        </div>
        <div class="col-lg-6 d-none d-lg-block">
          <div class="curso-image" style="background-image: url('/resources/img/cursos-livro.png')"></div>
        </div>
      </div>

      <div class="cursos-line"></div>

      <div style="padding: 47px 0px;">
        <div class="row">
          <div class="col-lg-4">
            <p class="cursos-advantages">
              16 Módulos divididos em 2 dias. O Curso mais Completo no Brasil sobre Hormônios na Prática Clínica. Curso com Turmas Nacionais e Internacionais (Ministrado na Europa). <br>
            </p>
          </div>
          <div class="col-lg-4">
            <p class="cursos-advantages">
              Aula Prática! Mediante inscrição são Solicitados Exames Laboratoriais para aula prática, onde os alunos desenvolverão a capacidade de interpretar seus próprios exames e atuar no equilíbrio e otimização de seus hormônios bem como dos demais parâmetros bioquímicos. <br>
            </p>
          </div>
          <div class="col-lg-4">
            <p class="cursos-advantages">
              Em breve Lançamento do Livro Hormônios - Do Empirismo à Terapia Individualizada.
            </p>
          </div>
        </div>
        <!-- <div class="d-flex justify-content-center">
          <button type="button" class="btn-cicle active" style="margin-right: 38px"></button>
          <button type="button" class="btn-cicle" style="margin-right: 38px"></button>
          <button type="button" class="btn-cicle"></button>
        </div> -->
      </div>
    </section>

    <section class="section-noticias" id="blog">
      <div class="container" style="padding: 35px 15px">
        <h1>
          Confira as<br>
          <span style="font-weight: bold">Nossas Notícias</span>
        </h1>

        <div id="noticias_container">
          <?php foreach ($noticias as $key => $value): ?>
            <div class="row" style="position: relative; <?= $key > 0 ? 'display: none' : ''; ?>">
              <?php foreach ($value as $noticia): ?>
                <div class="col-md-6 col-xl-3" style="margin-bottom: 45px">
                  <a class="noticia-box" href="/blog/<?= $noticia->id; ?>">
                    <div class="noticia-image" style="background-image: url('/uploads/capas/<?= $noticia->capa; ?>')"></div>
                    <div class="noticia-description">
                      <span class="noticia-title"><?= $noticia->titulo; ?> </span>
                      <?= str_replace("\n", '', substr(strip_tags($noticia->conteudo), 0, 100)); ?>
                    </div>
                  </a>
                </div>
              <?php endforeach; ?>
            </div>
          <?php endforeach; ?>
        </div>

        <script type="text/javascript">
          function exibirNoticias(i) {
            $('#noticias_container > div').hide();
            $('#noticias_container > div').eq(i).show();
            $('#noticias_buttons > button').removeClass('active');
            $('#noticias_buttons > button').eq(i).addClass('active');
          }
        </script>

        <div class="d-flex justify-content-center" id="noticias_buttons">
          <?php for ($n = 0; $n < count($noticias); $n++): ?>
            <?php if (count($noticias[$n]) > 0): ?>
              <button type="button" class="btn-cicle <?= $n == 0 ? 'active' : ''; ?>" onclick="exibirNoticias(<?= $n; ?>)" <?= $n == count($noticias) - 1 ? '' : 'style="margin-right: 38px"'; ?>></button>
            <?php endif; ?>
          <?php endfor; ?>
        </div>
      </div>
    </section>

    <section class="section-contato" id="contato">
      <div class="section-background" style="background-image: url('/resources/img/vector-bg.png')"></div>

      <div class="container" style="padding: 35px 15px;">
        <div class="row">
          <div class="col-xl-6 pr-xl-0">
            <h1>
              Entre em<br>
              <span style="font-weight: bold">Contato Conosco</span>
            </h1>

            <p>
              Você está a um passo de conquistar sua melhor versão.
              Converse diretamente com a gente pelo <strong>WhatsApp (83) 9 9859 9949</strong> ou se preferir, preencha o formulário e entraremos em contato.
            </p>

            <p class="text-danger">
              <?php echo validation_errors('<span class="d-block">', '</span>'); ?>
            </p>

            <form method="post">
              <div class="form-group">
                <label for="">Nome</label>
                <input type="text" class="form-control" name="nome" value="" placeholder="Seu nome">
              </div>
              <div class="form-group">
                <label for="">E-mail</label>
                <input type="email" class="form-control" name="email" value="" placeholder="Seu e-mail">
              </div>
              <div class="form-group">
                <label for="">Contato</label>
                <input type="text" class="form-control" name="contato" value="" placeholder="Telefone ou celular com DDD">
              </div>
              <div class="form-group">
                <label for="">Mensagem</label>
                <textarea class="form-control" name="mensagem" placeholder="Digite a sua mensagem aqui" style="height: 144px; resize: none;"></textarea>
              </div>
              <div class="text-right">
                <button type="submit" class="btn btn-primary" name="enviar_mensagem" value="true">Enviar</button>
              </div>
            </form>

            <p class="mt-5">
              <strong>Telefone/WhatsApp:</strong> +55 (83) 9 9859 9949 <br>
              <strong>E-mail:</strong> equeiroga.med@gmail.com <br>
              <strong>Endereço:</strong> Av. Guarabira, 200, Empresarial H Center, Sala 206 <br>
              Bairro Manaíra, João Pessoa - Paraíba <br>
              <strong>Horário de funcionamento:</strong>
              <span class="d-inline-block" style="vertical-align: top">
                Segunda a Sexta 08:00 às 20:00h <br>
                Sábado 08:00 às 12:00h
              </span>
            </p>

            <p>
              <a class="btn btn-social" href="https://www.facebook.com/drednaldoqueiroga" target="_blank"><i class="fab fa-facebook-square"></i></a>
              <a class="btn btn-social" href="https://www.instagram.com/drednaldoqueiroga/" target="_blank"><i class="fab fa-instagram"></i></a>
              <a class="btn btn-social" href="https://www.youtube.com/c/DrEdnaldoQueiroga" target="_blank"><i class="fab fa-youtube"></i></a>
            </p>
          </div>
        </div>
      </div>

      <div class="google-maps d-none d-xl-block">
        <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3959.1632516591185!2d-34.83190023597804!3d-7.1070713948676145!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x7acdd7eec8120c1%3A0x5c61542827a59b9f!2sCl%C3%ADnica%20Queiroga!5e0!3m2!1spt-PT!2sbr!4v1575501428862!5m2!1spt-PT!2sbr" width="600" height="450" frameborder="0" style="border:0;" allowfullscreen=""></iframe>
      </div>
    </section>

    <div class="contato-maps d-xl-none">
      <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3959.1632516591185!2d-34.83190023597804!3d-7.1070713948676145!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x7acdd7eec8120c1%3A0x5c61542827a59b9f!2sCl%C3%ADnica%20Queiroga!5e0!3m2!1spt-PT!2sbr!4v1575501428862!5m2!1spt-PT!2sbr" width="600" height="450" frameborder="0" style="border:0;" allowfullscreen=""></iframe>
    </div>

    <footer class="section-footer">
      <div class="container">
        <img src="/resources/img/vector-logo-footer.png" style="margin-right: 6px">
        COPYRIGHT &copy; 2017 - CLÍNICA QUEIROGA | MORIARTYDEV.COM
      </div>
    </footer>

    <!-- Optional JavaScript -->
    <!-- jQuery first, then Popper.js, then Bootstrap JS -->
    <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js" integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js" integrity="sha384-UO2eT0CpHqdSJQ6hJty5KVphtPhzWj9WO1clHTMGa3JDZwrnQq4sF86dIHNDz0W1" crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js" integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM" crossorigin="anonymous"></script>
    <script src="https://unpkg.com/sweetalert/dist/sweetalert.min.js"></script>

    <?php if ($this->input->get('msg') == 'error'): ?>
      <script type="text/javascript">
        swal('Ops!', 'Não foi possível enviar a sua mensagem, tente novamente!', 'error');
      </script>
    <?php endif; ?>

    <?php if ($this->input->get('msg') == 'success'): ?>
      <script type="text/javascript">
        swal('Pronto!', 'Sua mensagem foi enviada com sucesso, em breve entraremos em contato!', 'success');
      </script>
    <?php endif; ?>

    <div id="whatswidget-pre-wrapper" class="">
      <div id="whatswidget-widget-wrapper" class="whatswidget-widget-wrapper" style="all:revert;">
        <div id="whatswidget-conversation" class="whatswidget-conversation" style="display: none; opacity: 0;">
          <div class="whatswidget-conversation-header" style="all:revert;">
            <div id="whatswidget-conversation-title" class="whatswidget-conversation-title" style="all:revert;color:black;">
              WhatsApp
            </div>
          </div>
          <div id="whatswidget-conversation-message" class="whatswidget-conversation-message " style="all:revert;">
            Olá, Gostaria de marcar um horário.</div>
            <div class="whatswidget-conversation-cta" style="all:revert;">
              <a style="all:revert;" id="whatswidget-phone-desktop" target="_blank" href="https://web.whatsapp.com/send?phone=5583998599949" class="whatswidget-cta whatswidget-cta-desktop">
	               Enviar Mensagem
              </a>
              <a id="whatswidget-phone-mobile" target="_blank" href="https://wa.me/5583998599949" class="whatswidget-cta whatswidget-cta-mobile" style="all:revert;">
                Enviar Mensagem
              </a>
            </div>
            <a target="_blank" class="whatswidget-link" href="https://www.adamante.com.br/code/whatsapp-site/" title="Feito no WhatsWidget" style="all:revert;">
              <img src=" data:image/png;base64," style="all:revert;">
            </a>
          </div>
          <div id="whatswidget-conversation-message-outer" class="whatswidget-conversation-message-outer" style="all:revert;">
            <span id="whatswidget-text-header-outer" class="whatswidget-text-header-outer" style="all:revert;color:black;">
              Entre em contato agora mesmo via WhatsApp!
            </span>
            <br>
            <div id="whatswidget-text-message-outer" class="whatswidget-text-message-outer" style="all:revert;color:black;">

            </div>
          </div>
          <div class="whatswidget-button-wrapper" style="all:revert;">
            <div class="whatswidget-button" id="whatswidget-button" style="all:revert;">
              <div style="margin:0 auto;width:38.5px;all:revert;">
                <img class="whatswidget-icon" style="all:revert;" src="data:image/png;base64,">
              </div>
            </div>
          </div>
          <script id="whatswidget-script" type="text/javascript">
            document.getElementById("whatswidget-conversation").style.display="none";
            document.getElementById("whatswidget-conversation").style.opacity="0";
            var button = document.getElementById("whatswidget-button");
            button.addEventListener("click", openChat);
            var conversationMessageOuter = document.getElementById("whatswidget-conversation-message-outer");
            conversationMessageOuter.addEventListener("click", openChat);
            var chatOpen = !1;

            function openChat() {
                0==chatOpen?(document.getElementById("whatswidget-conversation").style.display="block", document.getElementById("whatswidget-conversation").style.opacity=100, chatOpen=!0, document.getElementById("whatswidget-conversation-message-outer").style.display="none"): (document.getElementById("whatswidget-conversation").style.opacity=0, document.getElementById("whatswidget-conversation").style.display="none", chatOpen=!1)
            }
          </script>
        </div>
      <style id="whatswidget-style">
        .whatswidget-widget-wrapper {
            font-family: "Helvetica Neue", "Apple Color Emoji", Helvetica, Arial, sans-serif !important;
            font-size: 16px !important;
            position: fixed !important;
            bottom: 20px !important;
            right: 30px !important;
            z-index: 1001 !important
        }

        .whatswidget-conversation {
            background-color: #e4dcd4 !important;
            background-image: url('data:image/png !important;base64,') !important;
            background-repeat: repeat !important;
            box-shadow: rgba(0, 0, 0, 0.16) 0px 5px 40px !important;
            width: 250px !important;
            height: 300px !important;
            border-radius: 10px !important;
            transition-duration: 0.5s !important;
            margin-bottom: 80px !important
        }

        .whatswidget-conversation-header {
            background-color: white !important;
            padding: 10px !important;
            padding-left: 25px !important;
            box-shadow: 0px 1px #00000029 !important;
            font-weight: 600 !important;
            border-top-left-radius: 10px !important;
            border-top-right-radius: 10px !important
        }

        .whatswidget-conversation-message {
            color: black;
            line-height: 1.2em !important;
            background-color: white !important;
            padding: 10px !important;
            margin: 10px !important;
            margin-left: 15px !important;
            border-radius: 5px !important
        }

        .whatswidget-conversation-message-outer {
            background-color: #FFF !important;
            padding: 10px !important;
            margin: 10px !important;
            margin-left: 0px !important;
            border-radius: 5px !important;
            box-shadow: rgba(0, 0, 0, 0.342) 0px 2.5px 10px !important;
            cursor: pointer !important;
            animation: nudge 2s linear infinite !important;
            margin-bottom: 70px !important
        }

        .whatswidget-text-header-outer {
            font-weight: bold !important;
            font-size: 90% !important
        }

        .whatswidget-text-message-outer {
            font-size: 90% !important
        }

        .whatswidget-conversation-cta {
            border-radius: 25px !important;
            width: 175px !important;
            font-size: 110% !important;
            padding: 10px !important;
            margin: 0 auto !important;
            text-align: center !important;
            background-color: #23b123 !important;
            color: white !important;
            font-weight: bold !important;
            box-shadow: rgba(0, 0, 0, 0.16) 0px 2.5px 10px !important;
            transition: 1s !important;
            position: absolute !important;
            top: 62% !important;
            left: 10% !important
        }

        .whatswidget-conversation-cta:hover {
            transform: scale(1.1) !important;
            filter: brightness(1.3) !important
        }

        .whatswidget-cta {
            text-decoration: none !important;
            color: white !important
        }

        .whatswidget-cta-desktop {
            display: none !important
        }

        .whatswidget-cta-mobile {
            display: inherit !important
        }

        @media (min-width: 48em) {
            .whatswidget-cta-desktop {
                display: inherit !important
            }
            .whatswidget-cta-mobile {
                display: none !important
            }
        }

        .whatswidget-button-wrapper {
            position: fixed !important;
            bottom: 15px !important;
            right: 15px !important
        }

        .whatswidget-button {
            position: relative !important;
            right: 0px !important;
            background-color: #31d831 !important;
            border-radius: 100% !important;
            width: 60px !important;
            height: 60px !important;
            box-shadow: 2px 1px #0d630d63 !important;
            transition: 1s !important
        }

        .whatswidget-icon {
            width: 42px !important;
            height: 42px !important;
            position: absolute !important;
            bottom: 10px !important;
            left: 10px !important;
        }

        .whatswidget-button:hover {
            filter: brightness(115%) !important;
            transform: rotate(15deg) scale(1.15) !important;
            cursor: pointer !important
        }

        @keyframes nudge {
            20%,
            100% {
                transform: translate(0, 0)
            }
            0% {
                transform: translate(0, 5px);
                transform: rotate(2deg)
            }
            10% {
                transform: translate(0, -5px);
                transform: rotate(-2deg)
            }
        }

        .whatswidget-link {
            position: absolute !important;
            bottom: 90px !important;
            right: 5px !important;
            opacity: 0.5 !important
        }
      </style>
    </div>
  </body>
</html>
